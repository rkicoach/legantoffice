...
class ProductDetail extends React.Component {
	...
	//	console.log('Form Data'+ JSON.stringify(data));
	   ...
	   axios.post('http://35.202.27.202/upload', data, config)
		  .then(response => {
							...
							<Box width={1 / 2}>
								{((data == 'Personalization') && (res_custitem_radio_imprint_options == "All" || res_custitem_radio_imprint_options == "With Personalization & Blank Only")) && 
									<Heading is='h3' fontSize='4' bold>
										{Number(custitem_pers_price) <= Number(custitem_price) ? 'Personalize - FREE!' : 'Personalize'} 
									</Heading>
								}
								{((data == 'Personalization') && (res_custitem_radio_imprint_options == "All" || res_custitem_radio_imprint_options == "With Personalization & Blank Only")) &&
									<Heading is='h5' fontSize='3' bold>
								...
								{resCountryCode != 'US' ? '' : 
									...
									<Box right p0>
										{(data == 'Personalization' && (res_custitem_radio_imprint_options == "All" || res_custitem_radio_imprint_options == "With Personalization & Blank Only")) ? <ShipTime
											brand={"Dacasso"}
											currentDate={new Date()}
											shippingCutoff={shipDate_lead ? shipDate_lead : new Date()}
							...
							{custitem_cur_avail_gnv == '0' || resCountryCode != 'US' ? null :
								<Box right p0>
									<ShipTime
										brand={"Dacasso"}
										currentDate={new Date()}
										shippingCutoff={shipDate_logo ? shipDate_logo : new Date()}
	...
	async getProd(data){
		...
		var requestURL = await fetch('http://cudcyzhoxl-dsn.algolia.net/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20vanilla%20JavaScript%20(lite)%203.24.7%3Breact-instantsearch%204.2.0%3BJS%20Helper%202.23.0&x-algolia-application-id=CUDCYZHOXL&x-algolia-api-key=ddced29ac732fb0e4f278bcdf96a8405', {
			method: 'post',
			headers: {
	...
	simlerDataItem() {
		return (
			(this.state.similarItem &&
				this.state.similarItem.map((data, i) => (
					i < 4 ?
						<Box key={'similer' + i} width={1/2} p1 onClick={this.getProd.bind(this,data)}>
							<NavItem className='link-block card' >
   ...
   //console.log('get all data'+ JSON.stringify(this));
    ...
    var jay = await fetch('http://cudcyzhoxl-dsn.algolia.net/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20vanilla%20JavaScript%20(lite)%203.24.7%3Breact-instantsearch%204.2.0%3BJS%20Helper%202.23.0&x-algolia-application-id=CUDCYZHOXL&x-algolia-api-key=ddced29ac732fb0e4f278bcdf96a8405', {
      method: 'post',
      headers: {
        'Content-Type': 'application/json'
      },
...
var response =await jay.json()
    ...
    /* axios.get('http://ip-api.com/json')
      .then(function (response) {
        ...
        var resCountryCodeVal = response.data.countryCode;
        self.setState({ resRegion: resRegionVal, resCountryCode: resCountryCodeVal, selectValue: resRegionVal })
	  }); */
	this.setState({ resRegion: "", resCountryCode: "US", selectValue: "" })  
	...
	this.setState({currentProdId:pID})
			...
			success: function (data) {
				...
				$.each(custofield, function (key, value) {
					...
					if (value.$attributes.scriptId == 'custitem_monogram_color_options') {
						custitem_monogram_color_options = value.value.name;
					}
				...
				this.setState({
					...
					custitem_price_online, custitem_teo_product_name, discountPrice, SubCategoryUrl, categoryUrl, SubCategoryName, categoryName, similarItem, itemtypealgolia, pageTitle, custitem_color, custitem_exterior_material, custitem_brand, custitem_warranty, custitem_Description, custitem_status, custitem_price, custitem_web_related_resources, custitem_prod_img_folder, storeDisplayName, arrDataProduct, custitem_collection, custitem_series, custitem_prd_size, custitem_pad_size, custitem_pad_design, custitem_pad_type, custitemfact_pack_type, countryofmanufacture, custitem_material_desc, custitem_care_clean, custitem_prd_weight, cartDataValue, custitem_logo_price, custitem_pers_price,custitem_radio_imprint_options, custitem_radio_logo_options, custitem_deboss_setup,custitem_uv_setup
				});
			}.bind(this),
			error: function (xhr, status, err) {
				console.log(err);
	...
	render() {
		...
		if (storeDisplayName != 'undefined') {
			for (var i = 0; i < datacount; i++) {
				...
				img = { 'original': imgNameUrl, 'thumbnail': imgNameUrl }
				images.push(img)
				counti++;
			}
		...
		const { resCountryCode, resRegion, custitem_cur_avail_gnv, outOfStockMessage,ItemInternalId,storeDetailedDescription,categoryUrl,custitem_ship_lead_time,custitem_monogram_color_options, custitem_pers_lead_time, custitem_monogram_style_options, custitem_logo_lead_time, custitem_logo_line_url, custitem_deboss_color_options, custitem_uv_example_url, custitem_deboss_example_url, custitem_price_online, custitem_teo_product_name, discountPrice, SubCategoryUrl, categoryUrlData, categoryName, SubCategoryName, custitem_pers_price, custitem_logo_price, custitem_prd_weight, cartDataValue, custitem_care_clean, custitem_material_desc, countryofmanufacture, custitemfact_pack_type, custitem_pad_design, custitem_pad_size, custitem_prd_size, custitem_series, custitem_collection, similarItem, itemtypealgolia, pageTitle, arrDataProduct, custitem_color, custitem_exterior_material, custitem_brand, custitem_warranty, custitem_Description, custitem_status, custitem_price, custitem_web_related_resources, custitem_pad_type,custitem_deboss_setup,custitem_uv_setup } = this.state;
	
		if (pageTitle == 'undefined' && pageTitle == null) {
			//window.location.href = menuhostUrl+'defaultcomponent';
		}
			...
			<div className='wrapper'>
					...
					<div className='breadcrumb'>
						<Box is='ul' fontSize='0.8em' p0 mb0>
							...
							<li>
								...
								<a href={'/shop/' + categoryUrl}>{categoryName}</a> <span>/</span>
							</li>
							<li>
								{/* <a href={menuhostUrl + 'shop/' + categoryUrl + '/' + SubCategoryUrl}>{SubCategoryName}</a> <span>/</span> */}
								<a href={'/shop/' + categoryUrl + '/' + SubCategoryUrl}>{SubCategoryName}</a> <span>/</span>
							</li>
							<li>
							...
							<div className='item-size'>
								<div className='personalization-wrap'>
									<Box width={1} css={style.personalise_wrap}>
										<Heading is="h4" center css={style.personalise_heading} px2>Free Ground Shipping - Orders $49+</Heading>
										{(itemtypealgolia == 'assemblyItem') ?
											...
											: <Flex is='label' width={1} py2 css={{ cursor: 'pointer', borderBottom: '1px solid #ccc' }}>
												...
												<Box width={1 / 2}>
													...
													<Box><Text is="span" css={style.rrp_price}>${(Number(custitem_price).toFixed(2))} </Text><Text is="span" color="#f00" ml2>{resDisinventory}%</Text></Box>
								...
								{arrDataProduct.map((data, i) => (
									data == 'Request a Swatch' ? <Text is='a' href="#" mb1 onClick={(e) => this.openSwatch(e)} key={i}>{data} <FaCaretRight /></Text> :
										<NavItem 
											key={i} 
											route={
												(data == 'Personalization Options') ? '/personalizationcomponent' : 
												(data == 'Conference Table Renderings') ? '/conferencecomponent' :
												(data == 'Types of Table Pads') ? '/tablepadcomponent' :
												(data == 'Desk Pad Styles') ? '/deskcomponent' :
												(data == 'Why Conference Table Pads') ? '/whyconferencecomponent' :
												menuhostUrl
											} 
											params={{ slug: 'abcdef' }}>
												{data} <FaCaretRight />
				...
				<Flex flexWrap='wrap'>
					...
					<Box width={4/10} className='product-review'>
						...
						<Yotpo productId={this.state.currentProdId}/>
						{custitem_collection && 
						<Flex 
						mb3 
							is="a" 
						...
						{/* end item-view-all */}
						{/* start similiar-item */}
						{this.simlerDataItem && this.simlerDataItem != "Null"?<div className='similiar-item'>
				...
				data={{ custitem_deboss_example_url, custitem_uv_example_url, custitem_logo_lead_time, custitem_logo_line_url, custitem_deboss_color_options, custitem_monogram_style_options, custitem_pers_lead_time, custitem_monogram_color_options }}
